...
from pansr.utils import box_ops


def sigmoid_focal_loss(inputs, targets, num_boxes, weights=None, alpha: float = 0.25, gamma: float = 2):
    """
...
class SetCriterion(nn.Module):
    ...
    def loss_labels_ce(self, outputs, targets, indices, num_masks):
        ...
        # Per-object cls weights
        target_cls_weights_o = torch.cat([t["cls_weights"][J] for t, (_, J) in zip(targets, indices)]).type(src_logits.dtype)
        target_cls_weights = torch.full(src_logits.shape[:2], 1., dtype=src_logits.dtype, device=src_logits.device)
        target_cls_weights[idx] = target_cls_weights_o

        loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight, reduction='none')

        loss_ce = (loss_ce * target_cls_weights).sum() / num_masks
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # Per-object cls weights
        target_cls_weights_o = torch.cat([t["cls_weights"][J] for t, (_, J) in zip(targets, indices)])
        target_cls_weights = torch.full(src_logits.shape[:2], 1., dtype=src_logits.dtype, device=src_logits.device)
        target_cls_weights[idx] = target_cls_weights_o
        ...
        loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes,
                                     weights=target_cls_weights.unsqueeze(2), alpha=self.focal_alpha, gamma=2) * src_logits.shape[1]
        losses = {'loss_ce': loss_ce}
