...
from ..transforms import CopyPaste
    ...
    def from_config(cls, cfg, is_train=True):
        ...
        if cfg.INPUT.COPY_PASTE.ENABLED:
            ...
            copy_paste = CopyPaste(
                ...
                rotation_range=cfg.INPUT.COPY_PASTE.ROTATION_RANGE,
                water_id=cfg.INPUT.COPY_PASTE.WATER_ID,
            )
    ...
    def __call__(self, dataset_dict):
        ...
        if "pan_seg_file_name" in dataset_dict:
            ...
            if self.copy_paste is not None:
                image, pan_seg_gt, segments_info = self.copy_paste(image, pan_seg_gt, segments_info)
                dataset_dict["image"] = torch.as_tensor(np.ascontiguousarray(image.transpose(2, 0, 1)))
                dataset_dict["segments_info"] = segments_info
            ...
            for segment_info in segments_info:
                ...
                if not segment_info["iscrowd"]:
                    ...
                    # Remove objects with 0 area. This can happen due to random cropping.
                    if mask.sum() < 1:
                        continue

                    classes.append(class_id)
                    masks.append(mask)
                    isthing.append(segment_info["isthing"] == 1)
                    w = segment_info["cls_weight"] if "cls_weight" in segment_info else 1.0
                    cls_weight.append(w)

            classes = np.array(classes)
            isthing = np.array(isthing)
            cls_weight = np.array(cls_weight)
            instances.gt_classes = torch.tensor(classes, dtype=torch.int64)
            instances.gt_isthing = torch.tensor(isthing, dtype=torch.bool)
            instances.gt_cls_weight = torch.tensor(cls_weight, dtype=torch.float16)
            if len(masks) == 0:
